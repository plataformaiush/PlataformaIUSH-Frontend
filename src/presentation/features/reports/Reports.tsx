...
const Reports = () => {
  ...
  const [periodEnrollments, setPeriodEnrollments] =
    useState<PeriodEnrollmentResponse>([]);
  const [moduleAttempts, setModuleAttempts] = useState<ModuleAttemptResponse>(
    [],
  );
  const [completionRate, setCompletionRate] =
    useState<CourseCompletionRateResponse>([]);
  const [activeVsInactive, setActiveVsInactive] =
    useState<ActiveVsInactiveCourses | null>(null);
  ...
  const enrollmentTrend =
    periodEnrollments.length > 1
      ? Math.round(
          ((parseInt(
            periodEnrollments[periodEnrollments.length - 1].total_inscripciones,
          ) -
  ...
  const activePercentage = activeVsInactive
    ? Math.round(
        (parseInt(activeVsInactive.activos) /
          parseInt(activeVsInactive.total_cursos)) *
          100,
      )
    : 0;
  ...
  const attemptsWithData = moduleAttempts.filter(
    (m) => m.promedio_intentos !== null,
  );
  ...
  const globalAverage =
    attemptsWithData.length > 0
      ? (
          attemptsWithData.reduce(
            (acc, m) => acc + parseFloat(m.promedio_intentos!),
            0,
  ...
  const maxAttempt =
    attemptsWithData.length > 0
      ? Math.max(
          ...attemptsWithData.map((m) => parseFloat(m.promedio_intentos!)),
        )
      : 1;
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p
                      ...
                      style={{ color: colors.textSecondary }}
                    >
                      {totalEnrollments.toLocaleString()} estudiantes
                      registrados
                    </p>
                  </div>
                  <select
                ...
                <div className="mt-4 flex flex-col items-start gap-2 text-sm">
                  <div
                    ...
                    style={{ color: colors.textBase }}
                  >
                    Tendencia {enrollmentTrend >= 0 ? "al alza" : "a la baja"}{" "}
                    de {Math.abs(enrollmentTrend)}%
                    <TrendingUp
                      className="h-4 w-4"
                      style={{ color: colors.primary }}
                    />
                  ...
                  <div
                    ...
                    style={{ color: colors.textSecondary }}
                  >
                    Mostrando inscripciones por periodo con agrupación{" "}
                    {agrupacion}.
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {activeVsInactive
                        ? parseInt(activeVsInactive.inactivos)
                        : 0}
                    </span>
                  </div>
                </div>
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  {attemptsWithData.slice(0, 3).map((module) => {
                    const pct = Math.round(
                      (parseFloat(module.promedio_intentos!) / maxAttempt) *
                        100,
                    );
                    return (
                      <div key={module.id_modulo}>
                        ...
                        >
                          ...
                          <span>
                            {parseFloat(module.promedio_intentos!).toFixed(1)}
                          </span>
                        </div>
                        <div
                ...
                <p
                  ...
                  style={{ color: colors.textOnDark, opacity: 0.6 }}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                >
                  {certificates
                    ? parseInt(certificates.total_emitidos).toLocaleString()
                    : "—"}
                </h2>
                ...
                <ResponsiveContainer width="100%" height={110}>
                  <BarChart
                    data={[
                      {
                        name: "Certificados",
                        Emitido: certificates
                          ? parseInt(certificates.total_emitidos)
                          : 0,
                        Descargado: certificates
                          ? parseInt(certificates.total_descargados)
                          : 0,
                      },
                    ]}
                    barCategoryGap="30%"
                    ...
                    <div key={course.curso_id}>
                      <div className="flex justify-between items-baseline mb-1">
                        ...
                        <span
                          ...
                          style={{
                            color: isHigh ? colors.primary : colors.tertiary,
                          }}
                        >
                          {pct.toFixed(0)}%
                        </span>
